...
;;; Returns true if World's Tetra's Bset's future position left/right comes in
;;; contact with World's Bset.
...
;;; Returns true if World's Tetra's Bset's future position rotation comes in
;;; contact with World's Bset.
(define (rotate-here? w)
  (cond [(empty? (tetra-blocks (world-tetra w))) #false]
        [else (or (rotate-parse (first (tetra-blocks (world-tetra w)))
                                ...
                                (posn-y (tetra-center (world-tetra w))))
                  ; Check if future rotated blocks go off left wall
                  (< (+ (* BLOCKSIZE (- 0 (block-y (first (tetra-blocks
                                                           (world-tetra w))))))
                        (posn-x (tetra-center (world-tetra w)))) 0)
                  ; Check if future rotated blocks go off right wall
                  (> (+ (* BLOCKSIZE (- 0 (block-y (first (tetra-blocks
                                                           (world-tetra w))))))
                        (posn-x (tetra-center (world-tetra w))))
                     (* GRID/COLS BLOCKSIZE))
                  ; Check if future rotated blocks go thru floor
...
;;; Checks if a specifc Tetra's Block future rotation would collide with any of
;;; the blocks in World's Bset.
...
;;; Consumes current world to remove any full rows from World Bset and drop all
;;; World Bset blocks above that row one grid row down
...
;;; Consumes World's Bset and the x/y coordinates of center of Tetra and adds
;;; them to World Bset.
...
;;; Consumes world state and returns new world if rotating the tetra does not
;;; collide with walls or other blocks.
...
;;; move-left/right : World Blocksize -> World
;;; Moves tetra left or right unless collision is detected, consumes world and
...
;;; Takes a Block structure and returns a fixed-size block with specified color
...
;;; Draws final end-game screen when World Bset extends above top of world.
(define (end-game w)
...
(check-expect (draw-pile (make-world (make-tetra (make-posn 90 190)
                                                 (list (make-block -1 0 "red")
                                                       ...
                                                       (make-block 0 0 "red")
                                                       (make-block 1 -1 "red")))
                                     (list (make-block 130 390 "blue")
                                           (make-block 150 390 "blue")
                                           (make-block 170 390 "blue")
                                           (make-block 190 390 "blue"))
                                     0)) .)
(check-expect (check-overflow (make-world
                               (make-tetra (make-posn 90 70)
                                           (list (make-block 0 0 "green")
                                                 (make-block 0 -1 "green")
                                                 (make-block -1 -1 "green")
                                                 (make-block -1 0 "green")))
...
(check-expect (check-overflow (make-world
                               (make-tetra (make-posn 90 70)
                                           (list (make-block 0 0 "green")
                                                 ...
                                                 (make-block -1 -1 "green")
                                                 (make-block -1 0 "green")))
                               (list (make-block 170 390 "red")) 0)) #false)
(check-expect (check-overflow (make-world
                               (make-tetra (make-posn 90 70)
                                           (list (make-block 0 0 "green")
                                                 (make-block 0 -1 "green")
                                                 (make-block -1 -1 "green")
                                                 (make-block -1 0 "green")))
                               (list (make-block 30 -10 "red")) 0)) #true)
(check-expect (end-game (make-world (make-tetra (make-posn 90 110)
                                                (list (make-block -1 0 "blue")
                                                      (make-block 0 0 "blue")
                                                      (make-block 1 0 "blue")
                                                      (make-block 2 0 "blue")))
